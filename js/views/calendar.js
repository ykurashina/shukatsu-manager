...
function renderDayDetail() {
  ...
  if (dayEvents.length === 0) {
    ...
    for (var ei = 0; ei < dayEvents.length; ei++) {
      ...
      eventsHTML += '<div class="day-event-item" style="border-left-color: ' + color + ';">'
        ...
        + '</div>'
        + '<button class="btn-icon-sm btn-icon-danger day-event-delete" data-cal-delete-id="' + ev.id + '" data-cal-delete-source="' + (ev.source || '') + '" data-cal-delete-title="' + escapeHTML(ev.title) + '" title="削除">'
  ...
  var todayLabel = DateUtils.isToday(selectedDate) ? ' （今日）' : '';

  panel.innerHTML = '<div class="calendar-day-detail-title">'
    + '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
    + '<rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>'
    + '<line x1="16" y1="2" x2="16" y2="6"></line>'
    + '<line x1="8" y1="2" x2="8" y2="6"></line>'
  ...
  // 削除ボタンのイベントリスナー
  var deleteBtns = panel.querySelectorAll('.day-event-delete');
  for (var di = 0; di < deleteBtns.length; di++) {
    (function(btn) {
      btn.addEventListener('click', function(e) {
        e.stopPropagation();
        var evId = btn.getAttribute('data-cal-delete-id');
        var evSource = btn.getAttribute('data-cal-delete-source');
        var evTitle = btn.getAttribute('data-cal-delete-title');
        Modal.confirm(
          'この予定「' + evTitle + '」を削除しますか？',
          function() {
            if (evSource === 'step') {
              Store.deleteStep(evId);
            } else if (evSource === 'event') {
